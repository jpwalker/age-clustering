...
from compute_nu_eff import calc_seljak_warren_w_cut, calc_mo_white_nu, calc_seljak_warren,\
    calc_sheth_tormen_nu, calc_sheth_tormen_nu2
...
#     sp_ax.plot(nu, b, 'k--')
    ...
    # Optimize Sheth-Tormen
    
    
    def min_func(params, (nu, b), cosmo):
        st = calc_sheth_tormen_nu2(nu, cosmo, a=params[0], p=params[1])
        return b - st[1]
    
    bf = leastsq(min_func, (0.707, 0.3), (overall_data,cosmo))
    (nu, b, _) = calc_sheth_tormen_nu(1000, cosmo, bf[0][0], bf[0][1])
    st_ax.plot(nu, b, 'r-', lw=4.)
    sp_ax.plot(nu, b, 'r-', lw=4.)
    show()